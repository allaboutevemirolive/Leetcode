public class B0037 {
    ...
    static int[] row_bitmap;
    static int[] col_bitmap;
    static int[] cube_bitmap;
    static int[] entry;
    static int[] sequence;
    // Always points to the first empty cell's SQUARE index, which is stored in
    // SEQUENCE
    static int seq_start;
    // Utility arrays to store mapping from SQUARE to ROW/COLs/CUBES: e.g. 37 ->
    // cube[1, 0], whose 1D index is 3;
    static int[] square_to_row;
    static int[] square_to_col;
    static int[] square_to_cube;
    ...
    public static void solveSudoku(char[][] board) {
        ...
        // Sequence stores all SQUARE indices of all cells, with 0..start-1 being all
        // filled cells, and the rest all empty
        // All cells initially all empty
        for (int i = 0; i < 81; i++)
            sequence[i] = i;
        for (int i = 0; i < 9; i++)
            for (int j = 0; j < 9; j++) {
                // Mapping from SQUARE to I/J is also beneficial: avoid calculating I/J from
                // SQUARE later
                int square = i * 9 + j;
        ...
        for (int i = 0; i < 9; i++)
            for (int j = 0; j < 9; j++)
                if (board[i][j] != '.') {
                    ...
                    // Compact non-empty cells to the front, and use SEQ_START to mark the first
                    // empty cell's position
    ...
    static boolean place(int seq_pos) {
        if (seq_pos >= 81)
            return true;
        int seq_next = nextPos(seq_pos);
        swapSeq(seq_pos, seq_next);
        int square = sequence[seq_pos], row_idx = square_to_row[square], col_idx = square_to_col[square],
                cube_idx = square_to_cube[square];
    ...
    // Find among empty cells the one with the smallest search space: least bits on
    // its bitmap;
    static int nextPos(int pos) {
        ...
        for (int i = pos; i < 81; i++) {
            ...
            int cell_bitmap = row_bitmap[square_to_row[square]] & col_bitmap[square_to_col[square]]
                    & cube_bitmap[square_to_cube[square]];
    ...
    static void swapSeq(int i, int j) {
        int tmp = sequence[i];
